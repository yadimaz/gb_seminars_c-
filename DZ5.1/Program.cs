...
class UserInputToCompileForTest
{
    ...
    {
        //Напишите свое решение здесь
        if ((x > 0 && x <= array.GetLength(0)) && (y > 0 && y <= array.GetLength(1))) return true;
        if (x < 1 || x > array.GetLength(0)) Console.WriteLine("Позиция по рядам выходит за пределы массива");
        if (y < 1 || y > array.GetLength(1)) Console.WriteLine("Позиция по колонкам выходит за пределы массива");
        return false;
    }
    ...
    {
        //Напишите свое решение здесь
        bool check = ValidatePosition(numbers, x, y);
        if (check == true) Console.WriteLine(FindElementByPosition(numbers, x, y));
    }
}
